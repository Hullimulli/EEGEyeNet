...
class AnalEyeZor():
    ...
    def __init__(self, task, dataset, preprocessing, models, electrodes = 1+np.arange(129),featureExtraction = False,
                 trainBool = True, saveModelBool = True, path=None, pathForOriginalRelativeToExecutable="./Joels_Files/"):
        """
        ...
        self.pathForOriginalRelativeToExecutable = pathForOriginalRelativeToExecutable
        ...
        for name, model in models.items():
            ...
            for i in range(self.numberOfNetworks):
                ...
                if config['task'] == 'LR_task':
                    ...
                    offset += self.binaryCrossEntropyLoss(dataY, prediction)
                elif config['task'] == 'Direction_task' and saveTrail == '_amplitude':
                    offset += self.meanSquareError(dataY, prediction)
                elif config['task'] == 'Direction_task' and saveTrail == '_angle':
                    offset += self.angleError(dataY, prediction)
                elif config['task'] == 'Position_task':
                    offset += self.euclideanDistance(dataY, prediction)
                else:
                ...
                for j in tqdm(range(int(dataShape[2]))):
                    ...
                    for i in range(self.numberOfNetworks):
                        ...
                        if config['task'] == 'LR_task':
                            if useAccuracyBool:
                                prediction = np.rint(prediction)
                            electrodeLosses[j] += self.binaryCrossEntropyLoss(dataY, prediction)
                        elif config['task'] == 'Direction_task' and saveTrail=='_amplitude':
                            electrodeLosses[j] += self.meanSquareError(dataY, prediction)
                        elif config['task'] == 'Direction_task' and saveTrail=='_angle':
                            electrodeLosses[j] += self.angleError(dataY, prediction)
                        elif config['task'] == 'Position_task':
                            electrodeLosses[j] += self.euclideanDistance(dataY, prediction)
    ...
    def electrodePlot(self, colourValues, filename="Electrode_Configuration", alpha=0.4):
        """
        ...
        @rtype:
        """
        pathForOriginalRelativeToExecutable = self.pathForOriginalRelativeToExecutable + "forPlot/"
    ...
    def moveModels(self, newFolderName, originalPath, getEpochMetricsBool=True):
        """
        Moves all important files from an AnalEyeZor generated folder to another. Useful to call directly after freshly training a new model.
    ...
    def plotTraining(self, modelFileName, filename='TrainingMetrics', columns=["Loss","Accuracy","Val_Loss","Val_Accuracy"],
                     savePlotBool=True, format="pdf"):
        """
    ...
    def topoPlot(self, values, filename='topoPlot', format='pdf', saveBool=True, cmap='Reds',epsilon=0.01):
        """
    ...
    def visualizePrediction(self, modelNames, nrOfruns = 5, nrOfPoints=9, filename='predictionVisualisation',
                            format='pdf',saveBool=True, scale=False):
        """
        Maps the ground truth to the euclidean space and plots the average prediction of all given architecture types. The variance is visualized with a circle.
        @param modelNames: Architectures which will be plotted.
        @type modelNames: List of Strings
        @param nrOfruns: Over how many runs the prediction is averaged.
        @type nrOfruns: Integer
        @param nrOfPoints: How many points are to be plotted. Always takes the first ones of the test set.
        @type nrOfPoints: Integer
        @param filename: Name of the output.
        @type filename: String
        @param format: Format of the output.
        @type format: String
        @param saveBool: If True, the plot will be saved instead of shown.
        @type saveBool: Bool
        ...
        @rtype:
        """
        dataY = IOHelper.get_npz_data(config['data_dir'], verbose=True)[1]
        ids = dataY[:, 0]
        trainIndices, valIndices, testIndices = split(ids, 0.7, 0.15, 0.15)
        if scale:
            dataX = IOHelper.get_npz_data(config['data_dir'], verbose=True)[0][:, :, self.electrodes.astype(np.int) - 1]
            logging.info('Standard Scaling')
            scaler = StandardScaler()
            scaler.fit(dataX[trainIndices])
            dataX = scaler.transform(dataX[valIndices])
        else:
            dataX = IOHelper.get_npz_data(config['data_dir'], verbose=True)[0][testIndices, :,:]
            dataX = dataX[:, :,self.electrodes.astype(np.int) - 1]
        valY = dataY[testIndices,1:]
        dataX, valY  = dataX[:nrOfPoints], valY[:nrOfPoints,:]
        del trainIndices, valIndices, testIndices, dataY
        if config['task'] == 'Direction_task':
            allpredictions = dict()
            ...
            for modelName in modelNames:
                ...
                for run in range(nrOfruns):
                    ...
                    path = config['checkpoint_dir'] + 'run' + str(run+1) + '/'
                    trainer.load(path)
                    prediction[run,:,:] = np.squeeze(trainer.predict(dataX))
                allpredictions[modelName] = prediction
            truth = valY
        else:
    ...
    def visualizePredictionDirection(self, modelNames, nrOfruns=5, nrOfPoints=9, filename='predictionVisualisation',
                            format='pdf', saveBool=True, scale=False, postfix=""):
        """
        Returns two plots. One visualizes the amplitude prediction, where the shift in the x-axis is done for clarity.
        Further it visualizes the angle prediction. The different radii are for clarity. For both cases, the variance
        is visualized with a line.
        @param modelNames: Architectures which will be visualized.
        @type modelNames: List of Strings
        @param nrOfruns: Over how many runs the prediction is averaged.
        @type nrOfruns: Integer
        @param nrOfPoints: How many points are visualized. Takes the n first samples of the test set.
        @type nrOfPoints: Integer
        @param filename: Name of the output.
        @type filename: String
        @param format: Format of the output.
        @type format: String
        @param saveBool: If True, the plot is saved instead of shown.
        @type saveBool: Bool
        @param scale: If True, the data is scaled.
        @type scale: Bool
        @param postfix: If "", the dataset is used. Else it looks for a custom signal within the customSignals folder generated with
        the customSignal function. It only uses StepDirection and takes the file with the appropriate postfix.
        @type postfix: String
        @return:
        @rtype:
        """
        pathForOriginalRelativeToExecutable = self.pathForOriginalRelativeToExecutable + "dimensionReductions/"
        ...
        if postfix=="":
            ...
            nrOfPoints=4
            dataX = np.load(pathForOriginalRelativeToExecutable + "customSignals/" + config['task'] + "_with_" + config['dataset'] + "_synchronised_" + config['preprocessing'] + "_" + "StepDirection" + postfix + ".npy")
        ...
        for i, modelName in enumerate(modelNames):
            ...
            sigmaAmp = np.std(allpredictions[modelName][:, :, 0], axis=0)
        ...
        for j in range(8):
            plt.plot(np.array([0, (len(modelNames)+2)*np.cos(centers[j])]),np.array([0, (len(modelNames)+2)*np.sin(centers[j])]), c='black', alpha=0.5)
    ...
    def generateTable(self,modelFileName,filename='tableLatex',addNrOfParams=True,caption="Performance of each Network",
        ...
        @param modelFileName: The csv filename within the model folder from which the table has to be generated.
        @type modelFileName: String
    ...
    def combineResults(self,modelFileName,directories,filename="Statistics",nameColumn="Model",
                       nameStartIndex=0,addNrOfParams=False):
        """
        Takes models from different folders and combines their statistics in one csv. This function is very limited. It only works for the statistics.csv output
        after training. The folders should have the same prefix, and the name of the model for the table as the rest of the name.
        @param modelFileName: Name of the statistics file within the folder.
        @type modelFileName: String
        @param directories: Directories which are to be taken.
        @type directories: List of Strings
        @param filename: Name of the output.
        @type filename: String
        @param nameColumn: What the column with the names is called.
        @type nameColumn: String
        @param nameStartIndex: How long the prefix is.
        @type nameStartIndex: Integer
        @param addNrOfParams: If True, model parameters are added.
        @type addNrOfParams: Bool
        @return:
        @rtype:
        """
        #Read data of first directory
        data = pd.read_csv(config['log_dir'] + directories[0] + modelFileName,index_col=None)

        if addNrOfParams:
            #Add Parameter of models in first directory
    ...
    def predictAll(self,postfix="",steps=4,run=1):

        """
        Predicts the entire dataset with all networks and saves the prediction in "x", ground truth in "y" and loss of each sample in "diff".
        For all losses except angle the euclidean distance is taken.
        @param postfix: End of the name of the save file.
        @type postfix: String
        @param steps: In how many steps the prediction is splitted (RAM reasoning).
        @type steps: Integer
        @param run: Which run is used for the prediction.
        @type run: Integer
        @return:
        @rtype:
        """

        pathForOriginalRelativeToExecutable = self.pathForOriginalRelativeToExecutable + "predictions/"
        ...
        for j in range(steps):
            ...
            for modelName in self.modelNames:
                ...
                if config['task'] == 'LR_task':
                    ...
                    path = config['checkpoint_dir'] + 'run' + str(run) + '/'
                    trainer.load(path)
                    predictions[modelName][j*pointsPerStep:(j+1)*pointsPerStep] = np.squeeze(trainer.predict(dataX))
                    losses[modelName][j * pointsPerStep:(j + 1) * pointsPerStep] = np.sqrt(np.subtract(
                        dataY[j * pointsPerStep:(j + 1) * pointsPerStep]**2,
                        predictions[modelName][j * pointsPerStep:(j + 1) * pointsPerStep]**2))
    ...
    def plotSignal(self, modelName, electrodes, nrOfLevels=4, colourMap='gist_rainbow', run=1, nrOfPoints=9,
                   ...
                   meanBool=True, componentAnalysis=None, dimensions=5,  plotMovementBool=False,
                   specificDataIndices=None, splitAngAmpBool = True, percentageThresh=0, scaleModification = 1,
                   offsetModification = 0,plotSignalsSeperatelyBool=False, dataType="", postfix=""):
        ...
        @param dataType: If not "", a custom signal from the customSignals folder is visualized. The name has to be according to the customSignal function.
        @type dataType: String
        @param postfix: Postfix of the custom signal savefile or activation maximization file.
        @type postfix: String
        @return:
        @rtype:
        """
    ...
    def movie(self,yValue,yDeviation=0,maxValue=100,slowDownFactor=10,filename='movie',cmap='Seismic'):
        """
        Returns a topoplot movie. Recommended for LR-Task only.
        @param yValue: Averages over signals which are regarding their absolute distance of their Y close to this value.
        @type yValue: float
        @param yDeviation: How far off the Y corresponding to the signal can be.
        @type yDeviation: float
        @param maxValue: Upper bound for the absolute value of the signal.
        @type maxValue: Float
        ...
        dataY = np.squeeze(IOHelper.get_npz_data(config['data_dir'], verbose=True)[1])
        dataX = np.squeeze(IOHelper.get_npz_data(config['data_dir'], verbose=True)[0][np.where(np.absolute(dataY[1:] - yValue) <= yDeviation)[0]])
        dataX = np.mean(dataX,axis=0)

        def update(i):
            im, cm = mne.viz.plot_topomap(np.squeeze(dataX[i]),electrodePositions[3:132,:], contours=0,vmin=-maxValue, vmax=maxValue,outlines=outlines,show=False,cmap=cmap)
            return im,
    ...
    def pca(self,scale=False):
        """
        Calculates the principle components for a dataset over the training data. Saves the matrix as a numpy file.
        @param scale: If True, the data is scaled.
        @type scale: Bool
        @return:
        @rtype:
        """
        pathForOriginalRelativeToExecutable = self.pathForOriginalRelativeToExecutable + "dimensionReductions/PCA/"
        trainY = IOHelper.get_npz_data(config['data_dir'], verbose=True)[1]
        ids = trainY[:, 0]
        trainIndices, valIndices, testIndices = split(ids, 0.7, 0.15, 0.15)
    ...
    def activationMaximization(self,modelName,run=1, filenamePostfix = "", steps=1024, componentAnalysis="",
                               dimensions=10,derivativeWeight = 1,referenceIndices=None,
                               referenceElectrodes=0,initTensor = "Zeros",epochs = 5):
        """
        Performs activation maximization. Recommended for LR-Task only.
        @param modelName: Name of the architecture.
        @type modelName: String
        @param run: Which run to use.
        @type run: Integer
        @param filenamePostfix: The ending of the filename.
        @type filenamePostfix: String
        @param steps: Number of gradient steps.
        @type steps: Integer
        @param componentAnalysis: Which component analysis is done on the reference input. Can be "PCA"
        @type componentAnalysis: String
        @param dimensions: How many components of the component analysis are used for reconstruction.
        @type dimensions: Integer
        @param derivativeWeight: How heavy non smooth signals should be penalized.
        @type derivativeWeight: Float
        @param referenceIndices: The activation maximization has a norm, which incooperates a 0&1 reference signal.
        The more it deviates, the higher the penalty. Has to be a 2 entry array.
        @type referenceIndices: Numpy Array
        @param referenceElectrodes: Which electrodes should be compared to the reference. The rest has a 2 norm regularization.
        @type referenceElectrodes: Numpy Array
        @param initTensor: The initial value for the maximization activation. Can be "Zeros", "Avg" or "Reference". Defaults to a random input.
        @type initTensor: String
        @param epochs: For how many times the procedure is redone. Takes the most confident signal in the end.
        @type epochs: Integer
        @return:
        @rtype:
        """
        pathForOriginalRelativeToExecutable = self.pathForOriginalRelativeToExecutable + "dimensionReductions/ActivationMaximization/"
    ...
    def pcaDimReduction(self,data,dim=2,transformBack = True):
        """
        Takes the signals and transforms them with the eigenvector matrix of the PCA. All values except the first n ones, which correspond
        to the eigenvectors with the highest eigenvalues, are set to zero. Then the signal is transformed back to its original space if desired.
        @param data: A [m,500,129] numpy array of all signals which have to be transformed.
        ...
        @rtype: [m,n or 500,129] Numpy Array
        """
    ...
    def findDataPoints(self, type = "UpToDown", yValue = 0, minimalStrength = 0,electrode = 32, scale=False,
                       componentAnalysis=None, dimensions=5, model="",run=1,postfix="",
                       lossThresh=0.1,returnAngleBool=False):
        """
        Returns specific indices within the dataset according to the defined type. Since this function combines many different methods, some arguments are only used with certain types.
        @param type: Type of the signal corresponding to the data indices. Can be "UpToDown", "DownToUp", "Misclassified", "UpAndDown", "LeftOnly", "RightOnly", "UpOnly", "DownOnly".
        @type type: String
        @param yValue: Y value of the LR-task which has to be extracted. Used in "UpToDown", "DownToUp".
        @type yValue: Integer
        @param thresh: The minimal Signal average that it can be considered as Up or Down. Used in "UpToDown", "DownToUp".
        @type thresh: Float
        @param electrode: Electrode on which the conditions are checked. Used in "UpToDown", "DownToUp".
        @type electrode: Integer
        @param scale: If True, the data is scaled. Used in "UpToDown", "DownToUp".
        @type scale: Bool
        @param componentAnalysis: Which component analysis is used to modify the signal. Used in "UpToDown", "DownToUp".
        @type componentAnalysis: String
        @param dimensions: How many components are used to reconstruct the signal. Used in "UpToDown", "DownToUp".
        @type dimensions: Integer
        @param model: Model of which the corresponding file generated by the predictAll function is used. Used in "Misclassified", "UpAndDown", "LeftOnly", "RightOnly", "UpOnly", "DownOnly".
        @type model: String
        @param run: The run of the model. Used in "Misclassified", "UpAndDown", "LeftOnly", "RightOnly", "UpOnly", "DownOnly".
        @type run: Integer
        @param postfix: The postfix of the predictAll generated filename. Used in "Misclassified". Used in "Misclassified", "UpAndDown", "LeftOnly", "RightOnly", "UpOnly", "DownOnly".
        @type postfix: String
        @param lossThresh: The minimal loss which has to be present in order to return the index. Used in "Misclassified", "UpAndDown", "LeftOnly", "RightOnly", "UpOnly", "DownOnly".
        @type lossThresh: float
        @param returnAngleBool: If True, the angular task is loaded. Used in "Misclassified".
        @type returnAngleBool: Bool
        @return:
        @rtype:
        """
        ...
        if type == "UpToDown":
            ...
            if scale:
                ...
                conditionIndices = np.argwhere(np.logical_and(meanStart > minimalStrength, meanEnd < minimalStrength))
                del trainX
                ...
                for i in tqdm(range(steps)):
                    ...
                    if conditionIndices is None:
                        ...
                        conditionIndices = np.concatenate((conditionIndices,int(1000*i) + np.argwhere(np.logical_and(meanStart > minimalStrength, meanEnd < minimalStrength))))
            else:
                ...
                conditionIndices = np.argwhere(np.logical_and(meanStart > minimalStrength, meanEnd < minimalStrength))
                del trainX
            intersect = np.intersect1d(conditionIndices, np.argwhere(trainY[:, 1] == yValue), return_indices=False)
            return intersect
        elif type == "DownToUp":
            trainY = IOHelper.get_npz_data(config['data_dir'], verbose=True)[1]
            if scale:
    ...
    def customSignal(self, type="Step", postfix="", noiseStd=0,amplitude=10,
                     turnPoint=150):
        """
        Generates a Numpy file with custom signals. The corresponding Y values are not generated.
        @param type: Which signal has to be generated. Can be "Step", "ContStep", "ContStepConfused", "TripleSlide", "Constant", "StepDirection".
        @type type: String
        @param postfix: Postfix which will be appended to the filename.
        @type postfix: String
        @param noiseStd: The standard deviation of the gaussian noise which is added upon the signal.
        @type noiseStd: Float
        @param amplitude: Amplitude of the signal.
        @type amplitude: Flaot
        @param turnPoint: At what sample the step occurs.
        @type turnPoint: Integer
        @return:
        @rtype:
        """
        pathForOriginalRelativeToExecutable = self.pathForOriginalRelativeToExecutable + "dimensionReductions/customSignals/"
    ...
    def attentionVisualization(self, modelName, filename, dataType="",method="Saliency",format="pdf",run=1,
                               componentAnalysis="",dimensions=10,dataIndices = np.asarray([0]),maxValue=100,
                               postfix="",artificialTruths=None, useAngleNetworkBool=False):
        """
        Plots a saliency map corresponding to a network over the signal of each electrode.
        @param modelName: Name of the architecture to be used.
        @type modelName: String
        @param filename: Basename of the output
        @type filename: String
        @param dataType: If not "" an artificial signal is used, either generated by customSignal or activationMaximization.
        @type dataType: String
        @param method: Which algorithm is used. Can be "Saliency" or "ScoreCam".
        @type method: String
        @param format: Format of the output files.
        @type format: String
        @param run: Which network run is used.
        @type run: Integer
        @param componentAnalysis: If signals of the dataset are used, they are modified with this component analysis method.
        @type componentAnalysis: String
        @param dimensions: How many components are used for reconstruction.
        @type dimensions: Integer
        @param dataIndices: From which samples a saliency map is generated.
        @type dataIndices: Numpy Array
        @param maxValue: Limit for the signal plot in mV.
        @type maxValue: Float
        @param postfix: Postfix of a custom signal filename.
        @type postfix: String
        @param artificialTruths: The truth corresponding to a non dataset signal. F.E. for the "Step" custom signal, one would put here np.array([0,1]).
        @type artificialTruths: Numpy Array
        @param useAngleNetworkBool: If True, returns the map for the angular task.
        @type useAngleNetworkBool: Bool
        @return:
        @rtype:
        """
