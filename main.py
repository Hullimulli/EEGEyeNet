...
def main():
    ...
    if local:
        ...
        def plot(path,task,filename,modelNames,colour,PFIIndexNames=None):
            ...
            if PFIIndexNames is None:
                for modelName in modelNames:
                    lossValues = pd.read_csv(asdf.currentFolderPath + 'PFI_'+filename+'.csv', usecols=["Average"]).to_numpy()
        ...
        plot("Position_All/","Position","Rand_Position",["PyramidalCNN"],"Greens")
        plot("Direction_All/", "Direction", "Rand_Amplitude", ["PyramidalCNN"], "Purples")
        plot("Direction_All/", "Direction", "Rand_Angle", ["PyramidalCNN"], "Blues")
        ...
        def transformData(path,model,task):
            ...
            #asdf.customSignal("StepDirection", amplitude=30, turnPoint=250, postfix="_Amp30", noiseStd=15)
            ...
            #asdf.visualizePredictionDirection(modelNames=["PyramidalCNN","CNN","InceptionTime","EEGNet","Xception"], nrOfPoints=9, nrOfruns=5,filename="Thesis_Step_Vis",postfix="_Amp30")
            ...
            #asdf.attentionVisualization(model,filename="Thesis_ActStep400",componentAnalysis="",method="Saliency",dimensions=1,run=1,dataIndices=indices,useAngleNetworkBool=False,artificialTruths=np.array([0,1]),postfix="_400_Noise",dataType="Step")
            #asdf.plotSignal('InceptionTime', electrodes,splitAngAmpBool=True,filename="Thesis_Step400",run=1,plotSignalsSeperatelyBool=False,specificDataIndices=indices,nrOfPoints=20000,nrOfLevels=4,meanBool=True,plotMovementBool=False,percentageThresh=3,maxValue=100,componentAnalysis="",dimensions=5,postfix="_400_Noise",dataType="Step")
            asdf.plotSignal(modelName="PyramidalCNN",electrodes=electrodes,filename="Avg_Signal",nrOfPoints=20000,maxValue=100)
            fdsa = 0

        transformData("LR_Top2_signalAnalysis/","PyramidalCNN","LR")
