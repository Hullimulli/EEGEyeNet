...
class LSTM(BaseNet):
    ...
    def __init__(self, model_name, path, loss, model_number, batch_size, input_shape, output_shape, kernel_size=64, epochs = 50, nb_filters=16, verbose=True,
        ...
        # Define the modules 
        self.lstm = nn.LSTM(input_size=self.input_channels, hidden_size=self.hidden_size, dropout=dropout, num_layers=3)
        #self.output_conv = nn.Conv1d(in_channels=self.input_channels, out_channels=self.output_channels, kernel_size=1)
